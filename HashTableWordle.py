...
class HashTableWordle():
    ...
    The hashed key uses the hash function, but the programs word_colour_pattern.py
    and guesser.py must be called after declaring $env:PYTHONHASHSEED=0 in
    Powershell, since this ensures the hash function gives the same hashkey over
    different sessions.
    ...
    def search_words(self, key):
        ...
        #hashed_key = str(key) #hash(key)) # in the Hashtable the key is stringified
